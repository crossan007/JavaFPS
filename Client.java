...
public class Client implements Runnable
{
	...
	public Client(Socket conn)  // this constructor takes a socket,
	//and establishes a object stream and gets the name and stuff like that
	{
		...
		try
		{
			...
			tempEvent=(Event)objSckIn.readObject();
			
			playerName=tempEvent.getSender();
			
			
			Debug.print("Name read");
			objSckOut.writeObject(new Event("Network-ICON",GameSettings.getPlayerName(),GameSettings.getAvatarFileName()));
	...
	public Client(String ipAddress,int port)//this constructor allows you to connect to another client with an ip address and a name
	...
	public void addActionListener(EventListener a)		//associates the client code with this class
	{
		...
		clientCodeEventListener.actionPerformed(tempEvent);
	}
	...
	public void run()
	{
		try
		{
			...
			{
				clientCodeEventListener.actionPerformed((Event)objSckIn.readObject());  //if an event is read, its is passed off to the client code
			}
		...
		catch(Exception e)		//oops error, kill the client
		{
			...
			clientCodeEventListener.actionPerformed(new Event("Network-SOff",playerName," "));
